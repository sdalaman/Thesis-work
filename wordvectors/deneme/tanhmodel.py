...
def test_model(model, inputs_pri, inputs_sec):
    ...
    f2 = open(compositionalMethod+"-1000-sent-vect-closest.txt", "w")
...
def train_model(fout,learning_rate,n_momentum):
    ...
    fname = "%s%s-1000-loss-values-%s.bin" % (path,compositionalMethod,prmStr)
...
corpusEng = Corpus(path,fnamePri)
corpusTr = Corpus(path,fnameSec)
corpusEng.data.cuda()
corpusTr.data.cuda()
...
momentlist = [0.2,0.5,0.7]
lrlist = [0.001]

ftestRes = open(compositionalMethod + "-1000-run.txt", "w")
...
for mmt in momentlist:
    for lr in lrlist:
        ...
        prmStr = "%s-%s-%d-%d" % (lrStr,mmtStr,max_epoch,batch_size)
        fullModelNamePth = path + compositionalMethod + '-' + 'model1000EnTr-'+prmStr+".pth"
        fullModelNamePck = path + compositionalMethod + '-' + 'model1000EnTr-'+prmStr+".pck"
        fullVocabFilePri = path + compositionalMethod + '-' + 'english.1000EnTr-'+prmStr+".vocab"
        fullVocabFileSec = path + compositionalMethod + '-' + 'turkish.1000EnTr-'+prmStr+".vocab"
