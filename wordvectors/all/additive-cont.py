...
def train_model(fout,learning_rate,n_momentum):
    ...
    prevLrStr = ("%2.15f" % prev_learning_rate).rstrip('0')
    prevMmtStr = ("%2.5f" % prev_momentum).rstrip('0')
    prevPrmStr = "%s-%s-%d-%d" % (prevLrStr, prevMmtStr, prev_last_epoch, batch_size)
    fullModelNamePth = "%s%s-%s-%s-%d.pth" % (path, compositionalMethod, modelName, prevPrmStr, prev_last_epoch)
    ...
    for epoch in range(prev_last_epoch,max_epoch):
        ...
        if (epoch+1) % threshold == 0 and epoch != prev_last_epoch:
            ...
            fh = open(fname, 'wb')  # Save file as pickle
